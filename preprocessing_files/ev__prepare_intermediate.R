...
data.df <- read.csv('/Users/francisco.avalos/Desktop/thesis/data/LA_2019_20.csv')
...
data.df$im_start <- strptime(data.df$im_start, format = '%Y-%m-%d %H:%M:%S')
data.df$single_im_day <- as.Date(data.df$single_im_day, format = '%Y-%m-%d')
data.df$im_end<- strptime(data.df$im_end, format = '%Y-%m-%d %H:%M:%S')
...
data.df$start_period <- as.Date(data.df$start_period, format = '%Y-%m-%d')

data.df$im_flag <- case_when(is.na(data.df$im_date) ~ "NO IM", TRUE ~ "IM")
...
stations.interested.df <- subset(station.df, !is.na(max))['station_id']
intermediate.data.of.interest.df <- merge(data.df, stations.interested.df, 
                                          by='station_id')
intermediate.data.of.interest.df <- intermediate.data.of.interest.df %>% 
  relocate(station_id, .before = station_latitude)


station.days.until <- intermediate.data.of.interest.df %>% 
  group_by(station_id, period_count) %>% 
  summarize(max.periods = max(day_count))

intermediate.data.of.interest.df <- 
  merge(intermediate.data.of.interest.df,
        station.days.until, by=c('station_id', 'period_count'))
intermediate.data.of.interest.df$remaining_days <- 
  intermediate.data.of.interest.df$max.periods - intermediate.data.of.interest.df$day_count
intermediate.data.of.interest.df <- 
  intermediate.data.of.interest.df %>% 
  relocate(station_id, .before = station_latitude)
intermediate.data.of.interest.df <- 
  intermediate.data.of.interest.df %>% 
  relocate(period_count, .after = max.periods)

intermediate.data.of.interest.df <- 
  intermediate.data.of.interest.df[order(intermediate.data.of.interest.df$station_id, intermediate.data.of.interest.df$flowdate), ]
...
# intermediate.data.of.interest.df$station_latitude <- as.double(intermediate.data.of.interest.df$station_latitude)
# intermediate.data.of.interest.df$station_longitude <- as.double(intermediate.data.of.interest.df$station_longitude)
# 
# station_lat.vec <- vector()
# station_lon.vec <- vector()
# stations <- unique(intermediate.data.of.interest.df$station_id)
# 
# for (i in stations) {
#   subset.cycle.df <- subset(intermediate.data.of.interest.df, station_id == i)
...
# }
# 
# intermediate.data.of.interest.df$station_latitude <- station_lat.vec
# intermediate.data.of.interest.df$station_longitude <- station_lon.vec


intermediate.data.of.interest.df$station_intances <- 
  paste(intermediate.data.of.interest.df$station_id, '-', intermediate.data.of.interest.df$running.im.now)
intermediate.data.of.interest.df$station_intances_adj <- 
  paste(intermediate.data.of.interest.df$station_id, '-', intermediate.data.of.interest.df$running.im.adj.now)
...
# # longitude = -77.062650
# 
# intermediate.data.of.interest.df$station_latitude[which(intermediate.data.of.interest.df$station_id==491)] <- 38.944057
# intermediate.data.of.interest.df$station_longitude[which(intermediate.data.of.interest.df$station_id==491)] <- -77.062650
# 
# 
# # station-id 927
# # # latitude = 32.880680
# # # longitude = -117.221240
# 
# intermediate.data.of.interest.df$station_latitude[which(intermediate.data.of.interest.df$station_id==491)] <- 38.944057
# intermediate.data.of.interest.df$station_longitude[which(intermediate.data.of.interest.df$station_id==491)] <- -77.062650

# intermediate.data.of.interest.df <- subset(intermediate.data.of.interest.df, !(station_id %in% c(491, 927)))
...
rm(v, rv, bucket.v, bucket.v10, n.periods.v, 
   ...
   running.day.adj, today.im.value, station.df, stations.interested.df, 
   station.days.until, stations, i, station_lon.vec, station_lat.vec, 
   subset.cycle.df, lat_length, lot_length, lat_min, lot_min, 
   min.im.minutes.threshold,
...
export.name <- 'intermediate_temps_stations.csv'
write.csv(intermediate.data.of.interest.df, paste0(path, export.name))
