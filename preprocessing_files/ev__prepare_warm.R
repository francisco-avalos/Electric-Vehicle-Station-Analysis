...
setwd('/Users/francisco.avalos/Desktop/thesis/data/hot.warm.locations/')
path <- '/Users/francisco.avalos/Desktop/thesis/data/hot.warm.locations/'
...
# ;
data.df[which((data.df$flowdate %in% c('2019-02-27', '2019-02-28','2019-02-29','2019-03-01','2019-03-02',
                ...
                data.df$station_id==864), ]$minutes_diff <- 1440
...
for (i in v) {
  ...
  individual.station.holder <- case_when(is.na(s.im.minutes) ~ 0, 
                                         s.im.minutes <= 360 ~ 1, 
                                         s.im.minutes <= 720 ~ 2, 
                                         s.im.minutes <= 1080 ~ 3, 
                                         s.im.minutes<= 1440 ~ 4)
  individual.station.holder.v10 <- case_when(is.na(s.im.minutes) ~ 0, 
                                             s.im.minutes <= 144 ~ 1, 
                                             s.im.minutes <= 288 ~ 2, 
                                             s.im.minutes <= 432 ~ 3, 
                                             s.im.minutes <= 576 ~ 4,
...
station.days.until <- warm.data.of.interest.df %>% 
  group_by(station_id, period_count) %>% 
  summarize(max.periods = max(day_count))

warm.data.of.interest.df <- 
  merge(warm.data.of.interest.df, station.days.until, by=c('station_id', 'period_count'))
warm.data.of.interest.df$remaining_days <- 
  warm.data.of.interest.df$max.periods - warm.data.of.interest.df$day_count
warm.data.of.interest.df <- 
  warm.data.of.interest.df %>% 
  relocate(station_id, .before = station_latitude)
warm.data.of.interest.df <- 
...
warm.data.of.interest.df <- 
  warm.data.of.interest.df[order(warm.data.of.interest.df$station_id, warm.data.of.interest.df$flowdate), ]
...
# warm.data.of.interest.df$station_latitude <- as.double(warm.data.of.interest.df$station_latitude)
# warm.data.of.interest.df$station_longitude <- as.double(warm.data.of.interest.df$station_longitude)
# 
# station_lat.vec <- vector()
# station_lon.vec <- vector()
# stations <- unique(warm.data.of.interest.df$station_id)
# for (i in stations) {
#   subset.cycle.df <- subset(warm.data.of.interest.df, station_id == i)
...
# }
# 
# warm.data.of.interest.df$station_latitude <- station_lat.vec
# warm.data.of.interest.df$station_longitude <- station_lon.vec


warm.data.of.interest.df$station_instances <- 
  paste(warm.data.of.interest.df$station_id, '-', warm.data.of.interest.df$running.im.now)
warm.data.of.interest.df$station_intances_adj <- 
  paste(warm.data.of.interest.df$station_id, '-', warm.data.of.interest.df$running.im.adj.now)
...
export.name <- 'warm_temps_stations.csv'
write.csv(warm.data.of.interest.df, paste0(path, export.name))
